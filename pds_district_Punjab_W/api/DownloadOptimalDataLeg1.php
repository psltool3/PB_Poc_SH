...
if (isset($_GET['format'])) {
	...
	$status_filter = isset($_GET['status']) ? strtolower(urldecode($_GET['status'])) : '';
	...
	if($type == "rollout" || $status_filter != ""){
		...
		$headers = ["Scenario","From","From_State","From_ID","From_Name","From_District","From_Centre","From_Lat","From_Long","WB_ID","WB_Lat","WB_Long","To","To_State","To_ID","To_Name","To_District","To_Centre","To_Lat","To_Long","Commodity","Quantity(Qtl)","Distance(Km)","Status"];

		$columns_pdf = ["scenario","from","from_id","from_name","from_district","from_lat","from_long","wb_id","wb_lat","wb_long","to","to_id","to_name","to_district","to_lat","to_long","commodity","quantity","distance","status"];
		$headers_pdf = ["Scenario","From","From_ID","From_Name","From_District","From_Lat","From_Long","WB_ID","WB_Lat","WB_Long","To","To_ID","To_Name","To_District","To_Lat","To_Long","Commodity","Quantity(Qtl)","Distance(Km)","Status"];
	} else {
		$columns = ["scenario","from","from_state","from_id","from_name","from_district","from_centre","from_lat","from_long","wb_id","wb_lat","wb_long","to","to_state","to_id","to_name","to_district","to_centre","to_lat","to_long","commodity","quantity","distance","approve_district","reason_district","approve_admin"];
		$headers = ["Scenario","From","From_State","From_ID","From_Name","From_District","From_Centre","From_Lat","From_Long","WB_ID","WB_Lat","WB_Long","To","To_State","To_ID","To_Name","To_District","To_Centre","To_Lat","To_Long","Commodity","Quantity(Qtl)","Distance(Km)","Implemented / Non Implemented","District Reason for not Implementing","Admin Approved"];

		$columns_pdf = ["scenario","from","from_id","from_name","from_district","from_lat","from_long","wb_id","wb_lat","wb_long","to","to_id","to_name","to_district","to_lat","to_long","commodity","quantity","distance","approve_district","reason_district","approve_admin"];
		$headers_pdf = ["Scenario","From","From_ID","From_Name","From_District","From_Lat","From_Long","WB_ID","WB_Lat","WB_Long","To","To_ID","To_Name","To_District","To_Lat","To_Long","Commodity","Quantity(Qtl)","Distance(Km)","Implemented / Non Implemented","District Reason for not Implementing","Admin Approved"];
	}
	...
	if(isset($_GET['month']) && $_GET['month'] != ""){
		...
		if(count($parts) >= 3){
			...
			$month_val = $parts[1];
			$day = $parts[2];
			$query = "SELECT * FROM optimised_table_leg1 WHERE year='$year' AND month='$month_val' AND day='$day'";
