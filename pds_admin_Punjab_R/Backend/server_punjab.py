        ...
        try:
            ...
            for row in wh_rows:
                data.append({
                    ...
                    'SW_Lat': row[4],
                    'SW_Long': row[5],
                    'Available_Capacity': row[8],
                    'Storage_Point_W': row[7],
            ...
            fci = fci.loc[:, [
                "PC_District", "PC_Name", "PC_ID", "PC_Lat", "PC_Long", "Wheat_Procure","Storage_Point_P",
            ]]
...
def save_to_database(month, year, day):
    ...
    if connection.is_connected():
        ...
        connection.commit()
        create_pc_query = ("CREATE TABLE " + pc_table + " (uniqueid VARCHAR(100) NOT NULL, PC_district VARCHAR(100) NOT NULL, PC_Name VARCHAR(100) NOT NULL, PC_Latitude VARCHAR(100) NOT NULL, PC_Longitute VARCHAR(100) NOT NULL, active VARCHAR(10) NOT NULL DEFAULT '1', PC_ID VARCHAR(100) NOT NULL, Paddy_Procure VARCHAR(100) NOT NULL, PC_Milling_Centre VARCHAR(100) NOT NULL)")
        ...
        create_mill_query = ("CREATE TABLE " + mill_table + " (Mill_District VARCHAR(100) NOT NULL, Mill_Name VARCHAR(100) NOT NULL, Mill_ID VARCHAR(100) NOT NULL, Mill_Latitute VARCHAR(100) NOT NULL,  Mill_Longitute VARCHAR(100) NOT NULL, Mill_Milling_Centre VARCHAR(100) NOT NULL,Milling_capacity VARCHAR(100) NOT NULL,uniqueid VARCHAR(100) NOT NULL,active VARCHAR(10) NOT NULL DEFAULT '1')")
...
def save_to_database_leg1(month, year, day):
    ...
    if connection.is_connected():
        ...
        connection.commit()
        create_pc1_query = ("CREATE TABLE " + pc1_table + " (uniqueid VARCHAR(100) NOT NULL, PC_District VARCHAR(100) NOT NULL, PC_Name VARCHAR(100) NOT NULL, PC_ID VARCHAR(100) NOT NULL,  PC_Lat VARCHAR(100) NOT NULL, PC_Long VARCHAR(100) NOT NULL, Storage_Point VARCHAR(100) NOT NULL,PC_Paddy VARCHAR(100) NOT NULL, active VARCHAR(10) NOT NULL DEFAULT '1')")
        cursor.execute(create_pc1_query)
        connection.commit()
        copy_pc1_data = ("INSERT INTO " + pc1_table + " SELECT * FROM pc2 WHERE active='1'")
        cursor.execute(copy_pc1_data)
        connection.commit()
        
        warehouse_drop_query = 'DROP TABLE IF EXISTS ' + warehouse_table;
        cursor.execute(warehouse_drop_query)
        create_warehouse_query = ("CREATE TABLE " + warehouse_table + " (district VARCHAR(100) NOT NULL, name VARCHAR(100) NOT NULL, id VARCHAR(100) NOT NULL, warehousetype VARCHAR(100) NOT NULL,  latitude VARCHAR(100) NOT NULL, longitude VARCHAR(100) NOT NULL,uniqueid VARCHAR(100) NOT NULL,Storage_Point_W VARCHAR(100) NOT NULL,Warehouse_Capacity_Available VARCHAR(100) NOT NULL,  active VARCHAR(10) NOT NULL DEFAULT '1' )")
        ...
        columns_to_fetch = ['Scenario','From','From_State','From_ID','From_Name','From_District','From_Centre','From_Lat','From_Long','To','To_State','WB_ID','WB_Lat','WB_Long','To_ID','To_Name', 'To_District', 'To_Centre','To_Lat', 'To_Long','commodity','quantity','Distance']
        df = pd.read_excel(excel_file_path)
        selected_data = df[columns_to_fetch]
        sql = 'DROP TABLE IF EXISTS ' + table_name;
        cursor.execute(sql)
        ...
        sql = "CREATE TABLE " + table_name + " ( scenario VARCHAR(150) NOT NULL, `from` VARCHAR(150) NOT NULL,from_state VARCHAR(150) NOT NULL, from_id VARCHAR(150) NOT NULL, from_name VARCHAR(150) NOT NULL, from_district VARCHAR(150) NOT NULL,from_centre VARCHAR(150) NOT NULL, from_lat VARCHAR(150) NOT NULL,from_long VARCHAR(150) NOT NULL, `wb_id` VARCHAR(150) NOT NULL,`wb_lat` VARCHAR(150) NOT NULL,`wb_long` VARCHAR(150) NOT NULL,`to` VARCHAR(150) NOT NULL,to_state VARCHAR(150) NOT NULL,to_id VARCHAR(150) NOT NULL, to_name VARCHAR(150) NOT NULL, to_district VARCHAR(150) NOT NULL, to_centre VARCHAR(150) NOT NULL, to_lat VARCHAR(150) NOT NULL, to_long VARCHAR(150) NOT NULL, commodity VARCHAR(150) NOT NULL,quantity VARCHAR(150) NOT NULL, distance VARCHAR(150) NOT NULL, approve_admin VARCHAR(100) , approve_district VARCHAR(100) , new_id_admin VARCHAR(100), new_id_district VARCHAR(100) , new_name_admin VARCHAR(100) , new_name_district VARCHAR(10) , reason_admin VARCHAR(255) , reason_district VARCHAR(255), new_distance_admin VARCHAR(100), new_distance_district VARCHAR(100), district_change_approve VARCHAR(100), status VARCHAR(100) )";
        cursor.execute(sql)
        connection.commit()
        
        for (index, row) in selected_data.iterrows():
            sql = 'INSERT INTO ' + table_name + ' (`scenario`, `from`, `from_state`, `from_id`, `from_name`, `from_district`,`from_centre`, `from_lat`, `from_long`,`wb_id`,`wb_lat`,`wb_long`, `to`, `to_state`, `to_id`, `to_name`, `to_district`,`to_centre`, `to_lat`, `to_long`, `commodity`, `quantity`, `distance`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s, %s, %s, %s, %s)'
            values = tuple(row)
            cursor.execute(sql, values)
            connection.commit()
...
def processFile():
    ...
    if total_demand > 0:
            ...
            for dist_name in districts:
                ...
                for i in FCI_d.index:
                    for j in FPS_d.index:
                        ...
                        phi_2 = math.radians(FPS_d["Mill_Lat"][j])

                        dphi = math.radians(FPS_d["Mill_Lat"][j] - FCI_d["PC_Lat"][i])
                        dlambda = math.radians(FPS_d["Mill_Long"][j] - FCI_d["PC_Long"][i])
                ...
                if Supply < Demand:
                    ...
                    # FPS cannot receive more than its capacity
                    for j in range(len(FPS_d)):
                        model += (
                            lpSum(Allocation[i][j] for i in range(len(FCI_d)))
                            <= FPS_d.loc[j, 'Milling_Capacity']
                        )

                    # All FCI supply must be allocated
                    for i in range(len(FCI_d)):
                        model += (
                            lpSum(Allocation[i][j] for j in range(len(FPS_d)))
                            == FCI_d.loc[i, 'Paddy_Procure']
                        )

                else:

                    # All mill capacity must be met
                    for j in range(len(FPS_d)):
                        model += (
                            lpSum(Allocation[i][j] for i in range(len(FCI_d)))
                            == FPS_d.loc[j, 'Milling_Capacity']
                    ...
                    # FCI allocation cannot exceed procurement
                    for i in range(len(FCI_d)):
                        model += (
                            lpSum(Allocation[i][j] for j in range(len(FPS_d)))
                            <= FCI_d.loc[i, 'Paddy_Procure']
                ...
                model.solve(PULP_CBC_CMD(msg=0, gapRel=0.3, timeLimit=120))
        ...
        df51.to_excel('Backend//Tagging_Sheet_Pre11.xlsx', sheet_name='BG_FPS1', index=False)
        ...
        input = pd.ExcelFile('Backend/Data_1.xlsx')
        node1 = pd.read_excel(input,sheet_name="A.1 Warehouse")
        node1["concatenate"]= node1['PC_Lat'].round(3).astype(str) + ',' + node1['PC_Long'].round(3).astype(str)
        
        node2 = pd.read_excel(input,sheet_name="A.2 FPS")
        node2["concatenate1"]= node2['Mill_Lat'].round(3).astype(str) + ',' + node2['Mill_Long'].round(3).astype(str)
        ...
        node1 = node1[['PC_ID', 'PC_Lat', 'PC_Long','concatenate']]
        War = pd.merge(node1, Warehouse, on='PC_ID')
        df1_w = War[War['concatenate'] != War['Lat_Long_r']]
        Warehouse_ID = df1_w['PC_ID'].unique()
        
        
        
        node2 = node2[['Mill_ID', 'Mill_Lat', 'Mill_Long','concatenate1']]
        node2['Mill_ID'] = node2['Mill_ID'].astype(str)
        FPS['Mill_ID'] = FPS['Mill_ID'].astype(str)
        FPS1 = pd.merge(node2, FPS, on='Mill_ID')
        df1_f = FPS1[FPS1['concatenate1'] != FPS1['Lat_Long_r']]
        FPS_ID = df1_f['Mill_ID'].unique()
        ...
        df10 = df10.drop(columns=["Tagging"], errors="ignore")
        result = ((df10['quantity']) * df10['Distance']).sum()
        
        df10 = pd.concat([df10], ignore_index=True)
...
# ----------------------------------------------------------------------------------------------------------
        ...
        input_file = 'Backend//Data_2.xlsx'
        input1 = pd.ExcelFile(input_file)
...
# ================= CHECK CONDITION =================
        if FCI['Wheat_Procure'].sum() > 0:
            ...
            FCI['Storage_Point_P'] = FCI['Storage_Point_P'].apply(lambda x: x.replace(' ', ''))
            FPS['Storage_Point_W'] = FPS['Storage_Point_W'].apply(lambda x: x.replace(' ', ''))
            ...
            columns_18 = [
                'Scenario','From','From_State','From_District','From_Storage_Point','From_ID','From_Name','From_Lat','From_Long',
                'To','To_ID','To_Name','To_State','To_District','To_Storage_Point','To_Lat','To_Long','Commodity','Quantity'
            ]
            ...
            if len(districts) > 0:
                ...
                for dist_name in districts:
                    ...
                    # Supply and Demand
                    Supply = FCI_d['Wheat_Procure'].sum()
                    Demand = FPS_d['Available_Capacity'].sum()
                    ...
                    if Supply < Demand:
                        ...
                        for i in range(len(FCI_d)):
                            model += (
                                lpSum(Allocation[i][j] for j in range(len(FPS_d)))
                                == FCI_d['Wheat_Procure'][i]
                            )

                    else:

                        # All FPS demand/capacity must be met
                        for j in range(len(FPS_d)):
                            model += (
                                lpSum(Allocation[i][j] for i in range(len(FCI_d)))
                                == FPS_d['Available_Capacity'][j]
                ...
                FCI_updated.loc[FCI_updated['Remaining_Wheat'] < 0,'Remaining_Wheat'] = 0
                ...
                # Sum allocation per warehouse
                wh_alloc1 = final_df.groupby('SW_ID')['Values'].sum().reset_index()
                wh_alloc1.rename(columns={'Values': 'Used_Allocation1'}, inplace=True)
                

                # Merge with original warehouse data
                FCI_updated1 = pd.merge(FPS, wh_alloc1, on='SW_ID', how='left')
                
               

                # Fill NaN with 0 (warehouses not used)
                FCI_updated1['Used_Allocation1'] = FCI_updated1['Used_Allocation1'].fillna(0)
                FCI_updated1['Remaining_Capacity1'] = (FCI_updated1['Available_Capacity'] - FCI_updated1['Used_Allocation1'])
                FCI_updated1.loc[FCI_updated1['Remaining_Capacity1'] < 0,'Remaining_Capacity1'] = 0
                ...
                df4 = pd.merge(df4, FPS, on='SW_ID', how='inner')
                df51 = df4[[
                    'PC_ID',
                    'PC_Name',
                    'PC_District',
                ...
                df51.insert(0, 'Scenario', 'Optimized')
                df51.insert(0, 'From', 'PC')
                df51.insert(2, 'From_State', 'Haryana')
                df51.insert(7, 'To', 'Depot')
                df51.insert(8, 'To_State', 'Haryana')
                df51.insert(9, 'Commodity', 'Paddy')
                df51.rename(columns={
                    'PC_ID': 'From_ID',
                    'PC_Name': 'From_Name',
                ...
                df51.to_excel('Backend//Tagging_Sheet_Pre12.xlsx', sheet_name='BG_FPS',index=False,)
            ...
            if FCI['Remaining_Wheat'].sum() > 0:
                ...
                for i in node1.index:
                    for j in node2.index:
                        ...
                        phi_2 = math.radians(node2["SW_Lat"][j])
                        delta_phi = math.radians(node2["SW_Lat"][j] - node1["PC_Lat"][i])
                        delta_lambda = math.radians(node2["SW_Long"][j] - node1["PC_Long"][i])
                ...
                for i in range(len(FCI)):
                    for j in range(len(FPS)):
                        Variable1.append(f"{FCI['PC_ID'][i]}_{FCI['Storage_Point_P'][i]}_{FPS['SW_ID'][j]}_{FPS['Storage_Point_W'][j]}_Wheat")
                ...
                if Supply > Demand:
                    ...
                    for i in range(len(FPS)):
                            model += lpSum(Allocation1[j][i] for j in range(len(FCI))) == FPS['Remaining_Capacity1'][i]
            ...
            df_combined.to_excel('Backend/Tagging_Sheet_Pre11.xlsx', index=False)
        ...
        # =========================================================
            ...
            empty_df.to_excel('Backend/Tagging_Sheet_Pre11.xlsx', index=False)

            print("✅ Empty final file created")
        
        
        dresult = pd.read_excel('Backend//Tagging_Sheet_Pre11.xlsx')

        xls = pd.ExcelFile('Backend//Data_2.xlsx')
        wb = pd.read_excel(xls, sheet_name='A.2 WB')
        print(wb)


        # Required columns
        dresult = dresult[
            ['From_ID', 'From_Lat', 'From_Long',
             'To_ID', 'To_Lat', 'To_Long']
        ]

        wb = wb[
            ['WB_ID', 'WB_Lat', 'WB_Long']
        ]
        ...
        # =====================================================
        # FIND BEST WB FOR EACH ROUTE
        # =====================================================

        results = []

        for _, row in dresult.iterrows():

            from_id = row['From_ID']
            from_lat = row['From_Lat']
            from_lon = row['From_Long']
            ...
            selected_wb = None
            selected_wb_lat = None
            selected_wb_lon = None

            # Check every WB
            for _, wb_row in wb.iterrows():

                wb_id = wb_row['WB_ID']
                wb_lat = wb_row['WB_Lat']
                wb_lon = wb_row['WB_Long']

                dist_from_to_wb = haversine(
                    from_lat,
                    from_lon,
                    wb_lat,
                    wb_lon
                ...
                dist_wb_to_destination = haversine(
                    wb_lat,
                    wb_lon,
                    to_lat,
                    to_lon
                ...
                if total_distance < min_distance:
                    ...
                    selected_wb = wb_id
                    selected_wb_lat = wb_lat
                    selected_wb_lon = wb_lon

                    selected_dist_from = dist_from_to_wb
                    selected_dist_to = dist_wb_to_destination
            ...
            results.append({
                ...
                'WB_Lat': selected_wb_lat,
                'WB_Long': selected_wb_lon,

                'Distance_From_To_WB_KM': round(selected_dist_from, 2),
                'Distance_WB_To_Destination_KM': round(selected_dist_to, 2),

                'Total_Route_Distance_KM': round(min_distance, 2)
            })
        ...
        # Save Output
        output_file = 'Backend//WB_Selected_Routes.xlsx'
        ...
        result_df1 = pd.read_excel('Backend//WB_Selected_Routes.xlsx')
        ...
        result_df1['Route_ID'] = (
            result_df1['From_ID'].astype(str) + '_' +
            result_df1['To_ID'].astype(str)
        )

        result_df2 = pd.read_excel('Backend//Tagging_Sheet_Pre11.xlsx')

        result_df2['Route_ID'] = (
            result_df2['From_ID'].astype(str) + '_' +
            result_df2['To_ID'].astype(str)
        )

        # Merge on Route_ID
        merged_df = pd.merge(
            result_df2,
            result_df1[['Route_ID', 'WB_ID','WB_Lat','WB_Long']],
            on='Route_ID',
            how='left'
        )
        ...
        merged_df = merged_df[[
                    'Scenario','From','From_State','From_District','From_Centre','From_ID','From_Name','From_Lat','From_Long','WB_ID','WB_Lat',
                    
                    'WB_Long','To','To_ID','To_Name','To_State','To_District','To_Centre','To_Lat','To_Long','Commodity','Quantity'
                ]]
                
        with pd.ExcelWriter('Backend//Tagging_Sheet_With_WB.xlsx', engine='openpyxl') as writer:
            merged_df.to_excel(
                writer,
                sheet_name='BG_FPS1',
                index=False
        ...
        input = pd.ExcelFile('Backend//Data_2.xlsx')
        node1 = pd.read_excel(input,sheet_name="A.2 WB")
        node1["concatenate"]= node1['WB_Lat'].round(3).astype(str) + ',' + node1['WB_Long'].round(3).astype(str)
        
        node2 = pd.read_excel(input,sheet_name="A.2 FCI")
        node2["concatenate1"]= node2['PC_Lat'].round(3).astype(str) + ',' + node2['PC_Long'].round(3).astype(str)
        ...
        node1 = node1[['WB_ID', 'WB_Lat', 'WB_Long','concatenate']]
        War = pd.merge(node1, Warehouse, on='WB_ID')
        df1_w = War[War['concatenate'] != War['Lat_Long_r']]
        Warehouse_ID = df1_w['WB_ID'].unique()
        
        node2 = node2[['PC_ID', 'PC_Lat', 'PC_Long','concatenate1']]
        node2['PC_ID'] = node2['PC_ID'].astype(str)
        FCI['PC_ID'] = FCI['PC_ID'].astype(str)
        FPS1 = pd.merge(node2, FCI, on='PC_ID')
        df1_f = FPS1[FPS1['concatenate1'] != FPS1['Lat_Long_r']]
        FPS_ID = df1_f['PC_ID'].unique()
        ...
        Distance3 = Distance2.drop(columns=Distance2.columns[Distance2.columns.isin(FPS_ID)])
        Distance3 = Distance3.T
        with pd.ExcelWriter('Backend//Punjab_Distance_L1.xlsx') as writer:
            Distance3.to_excel(writer, sheet_name='BG_BG',index=False)
            Warehouse.to_excel(writer, sheet_name='Warehouse', index=False)
            FCI.to_excel(writer, sheet_name='FCI', index=False)
        
        Cost=pd.ExcelFile('Backend//Punjab_Distance_L1.xlsx')
        BG_BG = pd.read_excel(Cost,sheet_name="BG_BG")
        Cost.close()
        
        data1 = pd.ExcelFile("Backend//Tagging_Sheet_With_WB.xlsx")
        df5 = pd.read_excel(data1,sheet_name="BG_FPS1")
        data1.close()
        print("shallu")
        ...
        df5["Tagging"] = df5['WB_ID'].astype(str) + '_' + df5['From_ID'].astype(str)
        df5['Distance1'] = df5['Tagging'].map(Distance_BG_BG)
        df5 = df5.replace('',pd.NaT).fillna('shallu')
        d5=df5.loc[df5['Distance1'] == "shallu"]
        df5.to_excel('Backend//Result_Sheet12_F1.xlsx',
                         sheet_name='Warehouse_FPS',index= False)
      
        
        
        Result_Sheet1 = pd.ExcelFile("Backend//Result_Sheet12_F1.xlsx")
        df6 = pd.read_excel(Result_Sheet1, sheet_name="Warehouse_FPS")

        # Filter rows where Distance == 'shallu'
        df7 = df6.loc[df6['Distance1'] == "shallu"]
        ...
        # Merge with old data
        df9 = df6.loc[df6['Distance1'] != "shallu"]
        df10 = pd.concat([df9, df7], ignore_index=True)
        ...
        # Compute total result
        df10["Distance1"] = df10["Distance1"].fillna(0)
        df10["quantity"] = df10["quantity"].fillna(0)
        df10 = df10.drop(columns=["Tagging"], errors="ignore")
        result = ((df10['quantity']) * df10['Distance1']).sum()
        
        df10 = pd.concat([df10], ignore_index=True)

        
        df10.to_excel('Backend//Result_Sheet_P1.xlsx', sheet_name='Warehouse_FPS', index=False)
        ...
        node2 = pd.read_excel(input,sheet_name="A.1 Warehouse")
        node2["concatenate1"]= node2['SW_Lat'].round(3).astype(str) + ',' + node2['SW_Long'].round(3).astype(str)
       
        DistanceBing = read_protected_excel('Backend//Distance_Intial_L3.xlsx', 'distf', sheet_name='BG_BG')
        Warehouse = read_protected_excel('Backend//Distance_Intial_L3.xlsx', 'distf', sheet_name='Warehouse')
        FCI = read_protected_excel('Backend//Distance_Intial_L3.xlsx', 'distf', sheet_name='FCI')
        ...
        node2 = node2[['SW_ID', 'SW_Lat', 'SW_Long','concatenate1']]
        node2['SW_ID'] = node2['SW_ID'].astype(str)
        FCI['SW_ID'] = FCI['SW_ID'].astype(str)
        FPS1 = pd.merge(node2, FCI, on='SW_ID')
        df1_f = FPS1[FPS1['concatenate1'] != FPS1['Lat_Long_r']]
        FPS_ID = df1_f['SW_ID'].unique()
        ...
        Distance3 = Distance2.drop(columns=Distance2.columns[Distance2.columns.isin(FPS_ID)])
        Distance3 = Distance3.T
        with pd.ExcelWriter('Backend//Punjab_Distance_L3.xlsx') as writer:
            Distance3.to_excel(writer, sheet_name='BG_BG',index=False)
            Warehouse.to_excel(writer, sheet_name='Warehouse', index=False)
            FCI.to_excel(writer, sheet_name='FCI', index=False)
        
        Cost=pd.ExcelFile('Backend//Punjab_Distance_L3.xlsx')
        BG_BG = pd.read_excel(Cost,sheet_name="BG_BG")
        Cost.close()
        
        data1 = pd.ExcelFile("Backend//Result_Sheet_P1.xlsx")
        df5 = pd.read_excel(data1,sheet_name="Warehouse_FPS")
        data1.close()
        print("shallu")
        ...
        df5["Tagging"] = df5['To_ID'].astype(str) + '_' + df5['WB_ID'].astype(str)
        df5['Distance2'] = df5['Tagging'].map(Distance_BG_BG)
        df5 = df5.replace('',pd.NaT).fillna('shallu')
        d5=df5.loc[df5['Distance2'] == "shallu"]
        df5.to_excel('Backend//Result_Sheet12_F2.xlsx',
                         sheet_name='Warehouse_FPS',index=False)
      
        
        
        Result_Sheet1 = pd.ExcelFile("Backend//Result_Sheet12_F2.xlsx")
        df6 = pd.read_excel(Result_Sheet1, sheet_name="Warehouse_FPS")

        # Filter rows where Distance == 'shallu'
        df7 = df6.loc[df6['Distance2'] == "shallu"]
        ...
        def process_single(row):
            ...
            token = get_token()
            if not token:
                print(f" No token for WB_ID={row['WB_ID']} → distance set to 0")
                return 0
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if (
                            ...
                            dist = single_json['data'][0]['distance']
                            print(f" WB_ID={row['WB_ID']} Distance={dist}")
                            return dist
                except Exception as e:
                    print(f" Error in single request ({row['WB_ID']}): {e}")

            print(f"⚠️ No distance for From_ID={row['WB_ID']} → set 0")
            return 0
        ...
        # Merge with old data
        df9 = df6.loc[df6['Distance2'] != "shallu"]
        df10 = pd.concat([df9, df7], ignore_index=True)
        ...
        # Compute total result
        df10["Distance2"] = df10["Distance2"].fillna(0)
        df10["quantity"] = df10["quantity"].fillna(0)
        df10 = df10.drop(columns=["Tagging"], errors="ignore")
        result = ((df10['quantity']) * df10['Distance2']).sum()
        ...
        df10['Distance'] = df10['Distance1'].fillna(0) + df10['Distance2'].fillna(0)

        # Drop old columns
        df10.drop(columns=['Distance1', 'Distance2'], inplace=True)
