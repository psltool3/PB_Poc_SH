 ...
 <style>
                ...
                <div class="page-content-wrap">                
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <!-- START WHEAT DATATABLE -->
                            <div class="panel panel-default">
								<div class="panel-heading">                                
                                    <h3 class="panel-title">Wheat (PC to Warehouse)</h3> 
                                </div>
								<div class="panel-body">
                                 <div class="table-responsive">
                                    <table id="export_table_wheat" class="table">
                                        <thead>
                                            <tr>
										...
										while($row = mysqli_fetch_array($result))
										{
											...
											echo "<tr><td>{$row['day']}</td>".
											 "<td>{$row['month']}</td>".
											 "<td>{$row['year']}</td>".
											 "<td> <button class='btn btn-info btn-rounded' onclick=\"wheat_pc_open('{$temp_id}')\">View PCs</button></td>".
             								 "<td> <button class='btn btn-warning btn-rounded' onclick=\"wheat_warehouse_open('{$temp_id}')\">View Warehouse</button></td>".
             								 "<td> <button class='btn btn-success btn-rounded' onclick=\"wheat_storage_open('{$temp_id}')\">View Storage Point</button></td>".
             								 "<td> <button class='btn btn-primary btn-rounded' onclick=\"wheat_weighbridge_open('{$temp_id}')\">View Weighbridge</button></td>".
             								 "<td> <button class='btn btn-danger btn-rounded' onclick=\"wheat_optimised_open('{$temp_id}')\">View Data</button></td>".
											 "<td> <button class='btn btn-danger btn-rounded' onclick=\"wheat_generate_report('{$temp_id}')\">View Report</button></td></tr>";
             							}
    ...
    <!-- START SCRIPTS -->
		...
		function milling_center_open(temp_id){
			post({id:temp_id} ,"../pds_admin_Punjab_R/MillingCenter.php");
		}
		
		function wheat_pc_open(temp_id){
			post({id:temp_id} ,"PcView_W.php");
		}
		
		function wheat_warehouse_open(temp_id){
			post({id:temp_id,step:"leg1"} ,"WarehouseView_W.php");
		}
		
		function wheat_storage_open(temp_id){
			post({id:temp_id,step:"leg1"} ,"StoragePoint_W.php");
		}
		
		function wheat_weighbridge_open(temp_id){
			post({id:temp_id,step:"leg1"} ,"Weighbridge_W.php");
		}
		
		function wheat_optimised_open(temp_id){
			post({id:temp_id,step:"leg1"} ,"OptimisedDataView_W.php");
		}
		
		function wheat_generate_report(temp_id, leg_id){
			post({id:temp_id,step:"all",legid:leg_id} ,"GenerateDataView_W.php");
		}
