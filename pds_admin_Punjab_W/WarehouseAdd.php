...
<script>
    ...
    <!-- START SCRIPTS -->
		...
		function showPopup() {
			...
			var numRegex = /^\d+(\.\d+)?$/;
			if (!numRegex.test(Capacity_Available) || parseFloat(Capacity_Available) < 0) {
				alert("Capacity Available must be a valid non-negative number.");
				document.getElementById("Capacity_Available").focus();
				return false;
			}
