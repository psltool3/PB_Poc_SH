...
<style>
	...
	* {
	...
	.dashboard-stat .stat-val {
		font-size: 20px;
		margin-bottom: 2px;
	}

	.dashboard-stat .stat-lab {
		font-size: 14px;
	}
</style>
...
<ul class="breadcrumb">
	<li><a href="#">Home</a></li>
	<li class="active">Punjab PDS Movement Optimisation</li>
</ul>
<!-- END BREADCRUMB -->
<div>
		...
		<div class="col-md-12">
			...
			<div class="panel panel-default">
				<div class="panel-heading" style="text-align: center;">
					...
					<h1 style="font-weight: bold; color: #FF6666;">Kindly Optimised the PC to Warehouse</h1>
						...
						<div class="col-md-9">
							<div class="input-group" style="width:100%;">
								<select class="form-control" id="type" name="type"
									...
									<option value='inter' selected style="font-weight:bold;color:#000;">Intra District
									</option>
								</select>
							</div>
				...
				<div class="col-md-12">
					<div class="panel-body">
						...
						<div class="row">
							<!-- Row 1 -->
							<div class="col-md-4">
								<div class="dashboard-stat" style="background-color:#4A90E2;">
									<div class="stat-val" id="total_pc_count"></div>
									<div class="stat-lab">Total PC</div>
								</div>
							</div>
							<div class="col-md-4">
								<div class="dashboard-stat" style="background-color:#28A745;">
									<div class="stat-val" id="total_district_count"></div>
									<div class="stat-lab">Total District</div>
								</div>
							</div>
							<div class="col-md-4">
								<div class="dashboard-stat" style="background-color:#E74C3C;">
									<div class="stat-val" id="total_mill_count"></div>
									<div class="stat-lab">Total Warehouse</div>
								</div>
							</div>
							...
							<div class="col-md-4">
								<div class="dashboard-stat" style="background-color:#3F51B5;">
									<div class="stat-val" id="milling_capacity_sum"></div>
									<div class="stat-lab">Warehouse Capacity</div>
								</div>
							</div>
						</div>
					...
					<div class="row">
						...
						<center style="margin-top:20px">
							<h4><b><span style="color: white;">State-Wise &nbsp <input type="checkbox"
											id="statewiseCheckbox" onchange="handleStateCheckboxChange()" checked /></b>
							</h4>
						</center>
...
<script>
		...
		$.ajax({
			...
			success: function (result) {
				...
				try {
					...
					document.getElementById("total_pc_count").innerHTML = "";
					document.getElementById("total_district_count").innerHTML = "";
					document.getElementById("total_mill_count").innerHTML = "";
					document.getElementById("procurement_paddy_sum").innerHTML = "";
					document.getElementById("milling_capacity_sum").innerHTML = "";
					...
					fetch(pythonUrl + 'extract_data', {
						...
						.then(data => {
							...
							fetch(pythonUrl + 'getfcidataleg1', {
								...
								.then(response => response.json())
								.then(data => {
									document.getElementById("total_pc_count").innerHTML = formatNumberWithCommasWithoutDecimal(data["Warehouse_No"]);
									document.getElementById("total_district_count").innerHTML = formatNumberWithCommasWithoutDecimal(data["District_Count"]);
									document.getElementById("total_mill_count").innerHTML = formatNumberWithCommasWithoutDecimal(data["FPS_No"]);
									document.getElementById("procurement_paddy_sum").innerHTML = formatNumberWithCommasWithoutDecimal(data["Total_Supply_Mota"]);
									document.getElementById("milling_capacity_sum").innerHTML = formatNumberWithCommasWithoutDecimal(data["Total_Demand Mota"]);
	...
	function handleOptimizationResult(data) {
		...
		var headers = [/*"Scenario",*/ "PC_Used", "Warehouse_Used", "Total_Allocation", "Total_QKM", "Average Distance"];
	...
	function handleStateCheckboxChange() {
		...
		if (checkbox.checked) {
			...
			fetch(pythonUrl + 'getGraphDataleg1', {
				...
				.then(response => response.json())
				.then(data => {
					data.District_Capacity = data.District_Capacity_Mota || {};
					data.District_Demand = data.District_Demand_Mota || {};
					
					console.log("District Capacity:", data.District_Capacity);
					console.log("District Demand:", data.District_Demand);

					data.District_Capacity1 = {};
					data.District_Capacity2 = {};
					data.District_Demand_Rice = {};
					data.District_Demand_Total = {};
					data.District_Paddy_Min = {};
					data.District_Name2 = { District_Name_All2: [] };
					data.District_Name3 = { District_Name_All3: [] };
					...
					var totalCapacity1 = 0;
					var totalCapacity2 = 0;
					var totalDemandPaddy = Object.values(data.District_Demand).reduce((acc, demand) => acc + demand, 0);
					var totalDemandRice = 0;
					var totalDemandTotal = 0;
					var totalIncomingPaddy = 0;
					var totalDemand = totalDemandPaddy;
					...
					var formattedTotalCapacity2 = 0;
					var formattedTotalIncoming = 0;
					
					var totalCapacityFinal = Object.values(data.District_Capacity_Mota || {})
						.reduce((acc, capacity) => acc + Number(capacity), 0);
					...
					document.getElementById("totalFciSupply").innerHTML =
						...
						formattedTotalCapacity3 + " (Qtl)</span>";

					document.getElementById("totalFciDemand").innerHTML =
						"<span style='color: white; font-size: 14px;'>Warehouse Capacity: " +
						formattedTotalDemand3 + " (Qtl)</span>";
					...
					if (totalCapacity > -1 && totalDemandPaddy > -1) {

						if (
							totalCapacity >-1 && totalDemandPaddy >-1
						...
						console.log("totalCapacity <= totalDemandPaddy",
							totalCapacity, "<=", totalDemandPaddy,
							totalCapacity <= totalDemandPaddy
						);
